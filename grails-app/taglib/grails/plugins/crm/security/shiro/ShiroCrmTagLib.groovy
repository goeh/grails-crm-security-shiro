...
class ShiroCrmTagLib {
    ...
    def userRoles = {attrs, body ->
        ...
        def username = attrs.username ?: SecurityUtils.subject?.principal?.toString()
        def result = ShiroCrmUserRole.createCriteria().list() {
            role {
                eq('tenantId', tenant)
            }
            user {
    ...
    def userPermissions = {attrs, body ->
        def tenant = attrs.tenant ?: TenantUtils.tenant
        def username = attrs.username ?: SecurityUtils.subject?.principal?.toString()
        def result = ShiroCrmUserPermission.createCriteria().list() {
            projections {
